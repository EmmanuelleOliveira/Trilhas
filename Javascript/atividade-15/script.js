...
function drawRow (printVector) { //Desenha a tabela com as informações inseridas
    let row = printVector.map(function(item){
    return `<tr>
                <td>${item.client}</td>
                <td>${item.date}</td>
                <td>${symbolMoney(item.price)}</td>
...
function filterInfo () { 
    if (debts.length === 0){
        document.getElementById("infoEmpty").innerHTML = "Insira as informações dos clientes primeiro";
    } else {
        document.getElementById("infoEmpty").innerHTML = "";
        let filterDate = [];
        let filterValue = [];
        ...
        if (initialDate !== "" && finalDate === ""){
            ...
            let dateFormat = (addZero((initialDate.getDate()+1).toString()) + "/" + (addZero(initialDate.getMonth()+1).toString()) + "/" + initialDate.getFullYear());
            let filtered = [];
            filterDate = debts.filter(function (item){
                return item.date >= dateFormat; 
            })
            filtered = drawRow(filterDate);
            document.getElementById("list").innerHTML = filtered.join("");
        } else if (finalDate !== "" && initialDate === ""){
            finalDate = new Date (finalDate);
            console.log (debts)
            let dateFormat = (addZero((finalDate.getDate()+1).toString()) + "/" + (addZero(finalDate.getMonth()+1).toString()) + "/" + finalDate.getFullYear());
            let filtered = [];
            filterDate = debts.filter(function (item){
                return item.date <= dateFormat; 
            })
            ...
            initialDate = new Date (initialDate);
            finalDate = new Date (finalDate);
            let initialDateFormat = (addZero((initialDate.getDate()+1).toString()) + "/" + (addZero(initialDate.getMonth()+1).toString()) + "/" + initialDate.getFullYear());
            let finalDateFormat = (addZero((finalDate.getDate()+1).toString()) + "/" + (addZero(finalDate.getMonth()+1).toString()) + "/" + finalDate.getFullYear());
            let filtered = [];
            filterDate = debts.filter(function (item){
                return (item.date >= initialDateFormat && item.date <= finalDateFormat); 
            })
            filtered = drawRow(filterDate);
            document.getElementById("list").innerHTML = filtered.join("");
        }
        if (valueMin > 0){
            let filtered = [];
            filterValue = debts.filter(function (item){
                return item.price >= valueMin;
            });
            filtered = drawRow(filterValue);
            document.getElementById("list").innerHTML = filtered.join("");
        } 
        if (valueMax > 0){
            let filtered = [];
            filterValue = debts.filter(function (item){
                return item.price <= valueMax;
            });
            filtered = drawRow(filterValue);
            document.getElementById("list").innerHTML = filtered.join("");
        }
